...
# Part 1: Execute the program and print the output sequence
outputs = []
registers = copy.deepcopy(registers_original)
instr = 0
while instr < len(programs):
    opcode = programs[instr]
    function = [adv, bxl, bst, jnz, bxc, out, bdv, cdv][opcode]
    num = programs[instr + 1]
    registers, instr = function(num, registers, instr, outputs)

answer = ','.join(str(num) for num in outputs)
print("Part 1 output:", answer)

# Part 2: Find the smallest valid 'a' for which the program produces the correct output sequence
def get_output(a):
    outputs = []
    registers = copy.deepcopy(registers_original)
    registers['A'] = a
